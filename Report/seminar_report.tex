...
To design a system that captures not only post-event footage but also pre-event video using a memory-efficient circular queue, ensuring a complete 40-second context (20 seconds before and after the event).\\
...
AVs are equipped with a variety of sensors such as cameras, LiDAR, RADAR, GPS, ultrasonic sensors, and inertial measurement units (IMUs). These sensors gather real-time data about the vehicle’s surroundings, including object detection, lane markings, traffic signals, and pedestrian movement. This data is critical for navigation, obstacle avoidance, and decision-making.
...
For the development of the Smart Vehicle Surveillance System, the Waterfall model was adopted as the primary software development methodology. The Waterfall model follows a linear and sequential approach, in which each phase of development is completed in full before progressing to the next. This approach ensures a structured workflow and clear milestones throughout the project lifecycle. The key phases followed in this project were:
\begin{enumerate} 
 ...
 \item Deployment
 \item Maintenance and Refinement
\end{enumerate}
\subsection{Requirement Analysis}
During this phase, all functional and technical requirements were identified and documented. Key requirements include:
\begin{itemize} 
\item Capturing 30 seconds of pre-event and post-event video.
\item Detecting suspicious activity using OpenCV.
\item Constructing a cloud-based API server to store video and metadata.
\item Developing a mobile application for video viewing and real-time notifications.
Mapping dependencies between modules to guide system design.
\end{itemize}
\subsection{System Design}In the system design phase, the architecture was finalized and the project was divided into four main modules:
\begin{itemize}
\item event app – handles event detection using OpenCV.
\item event manager – coordinates events and triggers video recording.
\item video manager – records video using a circular queue to store pre-\item event footage and encodes it using GStreamer.
\item cloud manager – handles cloud storage and metadata management.
\end{itemize}
Each module was designed to run as a separate Linux process and communicate through named pipes, allowing efficient parallel development and clear interface definitions.

 \subsection{Implementation}
The implementation phase involved the development of each module according to design specifications:
\begin{itemize}
\item video manager: Used GStreamer to encode and write MP4 files, maintaining a 30-second buffer of pre-event video.
\item event app: Simulated intelligent triggering via OpenCV-based suspicious person detection.
\item  cloud manager: Built with Apache, PHP, and MySQL, responsible for accepting uploads and storing metadata.
\item Android App: Developed to list and play recorded videos, and receive real-time notifications when new footage was uploaded.
Each module was implemented independently but strictly followed the design to ensure seamless integration during later phases.
\end{itemize}
\subsection{Integration  Testing}After implementation, all modules were integrated and tested end-to-end. Controlled events were simulated to validate:
\begin{itemize}
\item Event detection accuracy
\item Triggering of video recording
\item Successful upload to the cloud server
\item Real-time mobile notifications
\end{itemize}
Minor mismatches between modules, such as API format inconsistencies or timing issues, were identified and resolved during this phase.
\subsection{Maintenance and Refinement}
The final phase focused on minor improvements and performance enhancements based on testing outcomes:
\begin{itemize}
   \item Refinement of notification handling in the mobile app
   \item Optimization of video buffering in the video manager
   \item Improved error logging in the cloud manager
\end{itemize}
While the Waterfall model restricts revisiting earlier stages, necessary adjustments were made without major architectural changes.
\subsection{ Advantages and Limitations}
The Waterfall model provided a disciplined and structured approach suited to a system with well-defined objectives. It ensured careful planning, development, and testing, ultimately resulting in a well-integrated surveillance system.
However, limitations included:
\begin{itemize}
\item Early design decisions, such as the choice of detection algorithm or encoding settings, were difficult to change after implementation.
\item Limited scope for incorporating mid-project user feedback.
\end{itemize}
Despite these limitations, the Waterfall model was appropriate for this project, ensuring predictability, clear documentation, and orderly progression from requirement analysis to deployment.
%\vspace{10pt}
%\cleardoublepage
%\clearpage
...
%•\underline{Video Manager:} Upon receiving the trigger signal,the video manager performs the core video recording operation.It uses a circular buffer to continuously store the last 20 seconds of video data from the Camera Module,ensuring preevent footage is available. When activated, it extracts both the 20 seconds before the event and 20 seconds after,encodes it using GStreamer,and writes the footage into an MP4 file.This ensures comprehensive event capture with context.\\
...
\begin{enumerate}
 \item 
 \underline{\textbf{RealTime Event Detection}}
To implement an event detection mechanism using both external simulation and computer vision techniques (such as detecting suspicious persons using OpenCV) that can trigger the recording process.
\item \underline{\textbf{Buffered Video Recording}}
To design a system that captures not only post-event footage but also pre-event video using a memory efficient circular queue, ensuring a complete 40-second context (20 seconds before and after the event).\\
\textbf{Modular Component Design}}\\
To implement a modular architecture, where each subsystem (event app, event manager, video manager, cloud manager) runs as an independent process and communicates through Linux IPC mechanisms (named pipes).
\item \underline{\textbf{Cloud Upload System}}\\
To develop a locally hosted cloud backend using Apache, PHP, and MySQL, capable of storing uploaded videos and their associated metadata, accessible via RESTful APIs.
\item \underline{\textbf{Android Application Integration}}\\
To build an Android mobile application that: 
\end{enumerate}
\begin{itemize}
    \item 
Lists all recorded and uploaded videos,
...
Implemented in Python.
    \item 
 Simulated events (e.g., braking, honking) and real-world detection of       suspicious individuals using MediaPipe.
    \item 
 Human detection leveraged MediaPipe’s Pose and Holistic models to           identify persons within the camera frame.
    \item 
...
\begin{itemize}
  ...
  \item  Maintained a 20 second circular buffer of pre event frames.
  \item  Upon trigger, combined pre event and 20 second post event video into a      single MP4 file.
...
\end{figure}
\chapter{Conclusion}
The project successfully delivered a modular, event-driven video surveillance system that integrates real-time video recording, openCV-based suspicious person detection, buffered storage using a circular queue, and a cloud upload with mobile application with playback capabilities. Each module of the system was designed as an independent process communicating through Linux pipes, ensuring clean and decoupled architecture.
The video manager’s integration with Gstreamer enabled efficient handling of video data and accurate time segmented mp4 recording. The circular queue allowed the capture of 20 seconds of pre-event footage, a critical requirement in security applications.
The cloud manager facilitates automatic uploads to a locally hosted web server using PHP APIs, while the application provided users with immediate access to recordings using push notifications.\par
Despite challenges such as resource constraints, limited detection accuracy and the need for more robust fault handling, the system demonstrated a feasibility and practical value of combining embedded system, computer vision, cloud services and mobile app into an integrated surveillance system.
\chapter{References}
 \begin{itemize}
     \item Patel, M., Shah, A., & Kumar, R. (2024). Proposed method for car theft prevention techniques. International Journal of Technology Research and Studies, 12(3), 45-59.
     \item 
	 Rocky, A., Wu, Q. J., & Zhang, W. (2024). Review of accident detection methods using dashcam videos. IEEE Transactions on Intelligent Transportation Systems, 25(8), 8356-8367. 

\item  Rehman, M. T., Khan, N., & Ali Shah, S. (2018). Live view car surveillance system. IEEE Access, 6, 29488-29496. 
\item  Tesla, Inc. (2023). Sentry mode: Guarding your Tesla. https://www.tesla.com/blog/sentry-modeguarding-your-tesla 

\item Li, Y., Wang, J., & Chen, M. (2022). Edge AI for smart surveillance in intelligent transportation. Sensors, 22(5), 2105.
\item Ahmad, I., & Khan, M. A. (2022). Real-time data streaming in connected vehicles. Journal of Computer Networks and Communications, 2022, 1-11.
\item Consens : Collaborative and opportunistic sensing of road events by vehicles cameras(2023)
 \end{itemize}
 \usepackage{}
\pagestyle{fancy}
...
\fancyfoot[C]{\thepage}
\fancyfoot[R]{GEC Thiruvananthapuram}

% Optional lines
\renewcommand{\headrulewidth}{0.4pt}  % top line
\renewcommand{\footrulewidth}{0.4pt}  % bottom line
\end{document}