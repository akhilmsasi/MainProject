...
\begin{itemize} 
\item Capturing 30 seconds of pre-event and 30 seconds of post-event video.
\item Detecting suspicious activity using a Python-based computer vision pipeline built with OpenCV and MediaPipe.
...
\item Mapping dependencies between modules to guide system design.
\end{itemize}
\subsection{System Design}
In the system design phase, the architecture was finalized and the project was divided into four main modules:
\begin{itemize}
\item event app: handles trigger generation and computer-vision-based person detection.
\item event manager: coordinates events and initiates video recording.
\item video manager: records video using a circular queue to preserve pre-event footage and encodes clips using GStreamer.
\item cloud manager: handles cloud upload and metadata management.
\end{itemize}
...
\item event app: Simulated trigger events and performed person detection using OpenCV for video handling and MediaPipe for human-pose inference.
...
\item Each module was implemented independently but strictly followed the design to ensure seamless integration during later phases.
\end{itemize}
\subsection{Integration and Testing}
After implementation, all modules were integrated and tested end-to-end. Controlled events were simulated to validate:
\begin{itemize}
